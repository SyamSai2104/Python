...
def deposit():
    while True:
        amount = input("Enter your Deposit: Rs.")
        if amount.isdigit():
            amount = int(amount)
            if amount > 0:
                break
...
def get_bet():
    while True:
        amount = input(f"Enter your bet amount: ")
        if amount.isdigit():
            amount = int(amount)
            if MIN_BET <= amount <= MAX_BET:
                break
            else:
                print(f"Please enter amunt between {MIN_BET}-{MAX_BET}: ")
        else:
            print("Please enter a number")
    return amount
...
def spin(balance):
    ...
    while True:
        ...
        if total_bet > balance:
            print(f"No sufficient balance to bet, your balance is {balance}")
        else:
            break
...
def main():
    ...
    while True:
        ...
        option = input("Press enter to play(q to quit)").lower()
        if option == "q":
            break
